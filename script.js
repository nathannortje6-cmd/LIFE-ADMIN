...
// INIT LOCALSTORAGE
if(!localStorage.getItem('lifeadmin-users')) localStorage.setItem('lifeadmin-users',JSON.stringify({}));
...
function loadStats(){
    let users=JSON.parse(localStorage.getItem('lifeadmin-users'));
    const remindersCount=users[currentUser].reminders.length;
    const completedCount=users[currentUser].reminders.filter(r=>r.done).length;

...
function loadReminders(){
    ...
    users[currentUser].reminders.forEach((r,i)=>{
        ...
        info.classList.add('reminder-info');
        ...
        delBtn.classList.add('reminder-delete'); 
        ...
        if(delay>0){
            setTimeout(()=>{
                if(Notification.permission==="granted"){
                    new Notification("LIFE-ADMIN Reminder",{body:n.message});
                }
            }, delay);
        }
    });
...
function sendChat(){
    ...
    setTimeout(()=>{
        const reply="AI Suggestion: You could add '"+text+"' as a reminder!";
...
function loadEmails(){
    const emails = [
        {from:"bank@bank.com",subject:"Statement Ready"},
        {from:"tax@government.com",subject:"Tax Reminder"}
    ];
    const ul = document.getElementById('email-list'); ul.innerHTML='';
    emails.forEach(e=>{
        const li=document.createElement('li'); li.classList.add('reminder-card');
        li.innerHTML=`<div class="reminder-title">${e.subject}</div><div class="reminder-time">${e.from}</div>`;
        ul.appendChild(li);
    });
}

function loadBills(){
    const bills=[{title:"Electricity",amount:"R800",due:"2025-12-15"},{title:"Water",amount:"R200",due:"2025-12-10"}];
    const ul=document.getElementById('bill-list'); ul.innerHTML='';
    bills.forEach(b=>{
        const li=document.createElement('li'); li.classList.add('reminder-card');
        li.innerHTML=`<div class="reminder-title">${b.title} - ${b.amount}</div><div class="reminder-time">Due: ${b.due}</div>`;
        ul.appendChild(li);
    });
}

function loadTaxes(){
    const taxes=[{title:"Income Tax",amount:"R5000",due:"2025-12-31"}];
    const ul=document.getElementById('tax-list'); ul.innerHTML='';
    taxes.forEach(t=>{
        const li=document.createElement('li'); li.classList.add('reminder-card');
        li.innerHTML=`<div class="reminder-title">${t.title} - ${t.amount}</div><div class="reminder-time">Due: ${t.due}</div>`;
        ul.appendChild(li);
    });
}
